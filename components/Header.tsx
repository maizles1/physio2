      ...
      <div className="hidden md:block text-white py-2.5 bg-primary-gradient">
        <div className="container mx-auto px-4">
          <div
            className={
              hasSocial
                ? 'grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-x-3 sm:gap-x-4 text-sm'
                : 'grid grid-cols-2 items-center gap-x-3 sm:gap-x-4 text-sm'
            }
          >
            <div className="flex min-w-0 flex-nowrap items-center gap-3 sm:gap-4 justify-self-start">
              <a 
                ...
                className="flex min-h-[32px] shrink-0 items-center gap-2 hover:opacity-90 transition-opacity" 
                aria-label={`התקשר אלינו: ${phoneNumber}`}
              >
                <svg className="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                <span className="whitespace-nowrap font-medium">{phoneNumber}</span>
              </a>
              <span className="hidden text-blue-200 lg:inline" aria-hidden="true">|</span>
              <span className="hidden min-w-0 truncate text-white lg:inline">{address}</span>
            </div>
            {hasSocial && (
              <div className="flex shrink-0 items-center justify-center gap-2 justify-self-center">
                {seoConfig.social?.facebook && (
                  <a 
                    ...
                    className="flex h-8 w-8 items-center justify-center rounded-full border border-white/40 bg-white/30 hover:bg-white/40 transition-opacity hover:opacity-90" 
                    title="פייסבוק"
                  >
            ...
            <div className="flex min-w-0 items-center justify-self-end">
              <div className="flex items-center gap-2 text-white">
                ...
                <span className="whitespace-nowrap text-xs sm:text-sm">ראשון-חמישי: 08:00-20:00 | שישי: 08:00-14:00</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex min-w-0 items-center justify-between gap-3 py-2.5 sm:py-3 md:py-4">
          ...
          <Link href="/" className="flex shrink-0 items-center gap-3 hover:opacity-90 transition-opacity" aria-label="פיזיותרפיה.פלוס - דף בית">
            ...
            <div className="hidden min-w-0 sm:block">
              <span className="block text-lg font-bold leading-tight text-primary-dark xl:text-lg 2xl:text-xl">פיזיותרפיה.פלוס</span>
              <p className="hidden text-xs leading-tight text-primary 2xl:block">מקצוענות שמרגישים בגוף</p>
            </div>
          </Link>

          {/* דסקטופ רחב: ניווט מלא בלי חפיפה עם כפתורי יצירת קשר */}
          <div className="relative z-10 hidden min-w-0 flex-1 items-center gap-3 xl:flex 2xl:gap-5">
            <div className="flex min-w-0 flex-1 flex-nowrap items-center justify-center gap-x-0.5 2xl:gap-x-1">
              {primaryNavigation.map((item) => (
                ...
                >
                  <Link
                    href={item.href}
                    className="flex min-h-10 items-center justify-center whitespace-nowrap rounded-lg px-2 py-2 text-[13px] font-medium text-gray-700 transition-colors hover:bg-blue-50 hover:text-[#2080C0] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2080C0]/60 2xl:min-h-11 2xl:px-2.5 2xl:text-sm"
            ...
            <div className="flex shrink-0 items-center gap-1.5 border-s border-gray-200 ps-3 2xl:gap-2 2xl:ps-4">
              <a
                ...
                className="inline-flex h-10 shrink-0 items-center gap-1.5 rounded-lg bg-[#25D366] px-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#20BA5A] 2xl:px-3"
              ...
              {seoConfig.contact.email && (
                <a
                  ...
                  className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gray-600 text-white transition-colors hover:bg-gray-700 2xl:w-auto 2xl:gap-1.5 2xl:px-3"
                  aria-label={`שלח אימייל ל-${seoConfig.contact.email}`}
                >
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                  <span className="hidden 2xl:inline">אימייל</span>
                </a>
              )}
              <a
                href={`tel:${phoneNumber}`}
                ...
                className="inline-flex h-10 shrink-0 items-center gap-1.5 rounded-lg bg-[#2080C0] px-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#004080] 2xl:px-3"
                aria-label={`התקשר אלינו: ${phoneNumber}`}
              >
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                <span className="hidden 2xl:inline">התקשר</span>
              </a>
            </div>
          </div>
